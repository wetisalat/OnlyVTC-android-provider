package com.onlyvtc.driver.ui.bottomsheetdialog.invoice_show;
...
import com.onlyvtc.driver.data.network.model.ServiceType;
...
public class InvoiceShowDialogFragment extends BaseBottomSheetDialogFragment {
    ...
    private void init() {
        if (DATUM_history_detail != null) {
            ...
            if (payment != null) {
                lblBasreFare.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(payment.getFixed() + "")));
                lblTax.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(payment.getTax() + "")));
                if (payment.getTips() == 0 || payment.getTips() == 0.0)
                    tipsLayout.setVisibility(View.GONE);
                else {
                    tipsLayout.setVisibility(View.VISIBLE);
                    lblTips.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(payment.getTips() + "")));
                }
                Double pastTripTotal = payment.getTotal() + payment.getTips();
                lblTotal.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(pastTripTotal + "")));


                if (payment.getPayable() > 0) {
                    llAmountToBePaid.setVisibility(View.VISIBLE);
                    lblPaid.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(payment.getPayable() + "")));
                ...
                if (serviceType != null) {
                    ...
                    switch (serviceCalculator) {
                        case Constants.InvoiceFare.MIN:
                            ...
                            lblTimeFare.setText(MvpApplication.getInstance().getNewNumberFormat(payment.getMinute()));
                            break;
                        case Constants.InvoiceFare.HOUR:
                            distanceContainer.setVisibility(View.GONE);
                            timeContainer.setVisibility(View.GONE);
                            lblTimeFare.setText(MvpApplication.getInstance().getNewNumberFormat(payment.getHour()));
                            break;
                        case Constants.InvoiceFare.DISTANCE:
                            distanceContainer.setVisibility(View.GONE);
                            timeContainer.setVisibility(View.GONE);
                            lblDistanceFare.setText(MvpApplication.getInstance().getNewNumberFormat(payment.getDistance()));
                            break;
                ...
                lblDistanceFare.setText(MvpApplication.getInstance().getNewNumberFormat(Double.parseDouble(payment.getDistance() + "")));
            }
        }
    }
